...
void ig_read_file(ig_context* context, const char* filename, void** data_out, size_t* size_out) {
	#ifndef IG_ANDROID
	...
	*data_out = malloc(*size_out);
	fread(*data_out, 1, *size_out, file);
	fclose(file);
	#else
	AAsset* asset = AAssetManager_open(context->asset_manager, filename, AASSET_MODE_BUFFER);
    if (!asset) {
        IG_LOG("error reading file: \'%s\'\n", filename);
        exit(-1);
    }

    *size_out = AAsset_getLength(asset);
    
    *data_out = malloc(*size_out);
    AAsset_read(asset, *data_out, *size_out);
    AAsset_close(asset);
	#endif
}
...
VkShaderModule ig_context_create_shader_from_file(ig_context* context, const char* file_name) {
	...
	ig_read_file(context, file_name, &content, &file_size);
...
ig_context* ig_context_create(ig_window* window, float resolution, int vsync) {
	...
	r->asset_manager = window->native_handle->activity->assetManager;

	unsigned int extensions_count = 2;
	...
	vkCreateAndroidSurfaceKHR(r->instance, &(VkAndroidSurfaceCreateInfoKHR) {
		.sType = VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR,
		.pNext = NULL,
		.flags = 0,
		.window = window->native_handle->window
	}, NULL, &r->surface);
	...
	for (int i = 0; i < device_count; i++) {
		...
		for (int j = 0; j < format_count; j++) {
			if (formats[j].colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR && formats[j].format ==
				#ifndef IG_ANDROID
		...
		if (selected_queue != -1 && selected_format != -1 && supports_swapchain && supports_fifo && supports_non_fifo) {
			...
			#else
			IG_LOG("selected card: %s, max api version %d.%d.%d, running %d.%d.%d\n", device_props.deviceName,
				VK_API_VERSION_MAJOR(device_props.apiVersion),
				VK_API_VERSION_MINOR(device_props.apiVersion),
				VK_API_VERSION_PATCH(device_props.apiVersion),
				VK_API_VERSION_MAJOR(VK_API_VERSION_1_0),
	...
	vkCreateSwapchainKHR(r->device, &(VkSwapchainCreateInfoKHR) {
		...
		.pQueueFamilyIndices = (uint32_t*) &selected_queue,
...
bool ig_context_begin(ig_context* context, ig_window* window, float resolution, int vsync) {
	...
	if (acquire_result == VK_ERROR_OUT_OF_DATE_KHR) {
		...
		#else
		IG_LOG("failed to acquire next image.\n");
		#endif
		exit(-1);
	}
