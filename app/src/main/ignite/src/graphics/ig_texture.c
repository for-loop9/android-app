...
ig_texture* ig_context_texture_create_from_file(ig_context* context, const char* filename) {
	ig_ivec2 img_dim;
	int c;
	stbi_uc* img_data;
	#ifndef IG_ANDROID
	img_data = stbi_load(filename, &img_dim.x, &img_dim.y, &c, 4);
	#else
	void* full_buf;
	size_t full_sz;
	ig_read_file(context, filename, &full_buf, &full_sz);
	img_data = stbi_load_from_memory((stbi_uc*) full_buf, full_sz, &img_dim.x, &img_dim.y, &c, 4);
	free(full_buf);
	#endif
...
void ig_context_texture_destroy(ig_context* context, ig_texture* image) {
